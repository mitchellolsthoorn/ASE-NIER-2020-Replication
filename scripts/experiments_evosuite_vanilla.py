...
def getEvoSuiteCall(seed, configId, config, project, clazz, id, strategy, coreIndex):
  ...
  result += ' -Dalgorithm=DynaMOSA'
  result += ' -Dsearch_budget=120'
  result += ' -Dassertion_timeout=120'
  result += ' -Dminimization_timeout=120'
  result += ' -Dconfiguration_id=EvoSuite_vanilla'
  result += " -projectCP " + class_path
  result += ' -Doutput_variables=TARGET_CLASS,criterion,configuration_id,Random_Seed,Total_Goals,Total_Branches,Lines,Covered_Goals,Generations,Statements_Executed,Fitness_Evaluations,Tests_Executed,Generations,Total_Time,Size,Result_Size,Length,Result_Length,Total_Branches_Real,Coverage,BranchCoverage,LineCoverage,WeakMutationScore'
  result += " -Dgroup_id="+project
  result += " -Dshow_progress=FALSE"
  result += " -Dfuzzer=FALSE"
  result += " "+config+" "+FIXED
